/**
 * Adapted from https://github.com/photonstorm/typescript-signals
 */

type Listener<T extends any[] = any[], K = any> = (...args: T) => K

export class SignalBinding<T extends any[] = any[], K = any> {
  /** Handler function bound to the signal. */
  private _listener: Listener<T, K>
   ...
   */
  public execute(paramsArr?: T): K | undefined {
    if (this.active && !!this._listener) {
      const params = this.params ? this.params.concat(paramsArr) : paramsArr

      const handlerReturn = this._listener.apply(this.context, params)
